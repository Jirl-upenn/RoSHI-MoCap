...
class Args:
    ...
    name: str | None = None
    """Optional full output filename stem (without extension)."""
    overwrite_output: bool = False
    """Whether to overwrite an existing output file with the same name."""
...
def main(args: Args) -> None:
    ...
    # Auto-detect full sequence length when traj_length == -1
    max_length = len(transforms.pose_timesteps) - args.start_index - 1
    if args.traj_length <= 0:
        args.traj_length = max_length
        print(f"Using full sequence: {args.traj_length} frames")
    else:
        args.traj_length = min(args.traj_length, max_length)
    ...
    if traj_paths.hand_tracking_results_csv is not None:
        ...
        aria_all_hand_detections = CorrespondedAriaHandAllPoseWrtWorld.load(
            traj_paths.hand_tracking_results_csv,
            pose_timestamps_sec,
    ...
    if args.save_traj:
        if args.name is not None and args.name.strip() != "":
            save_name = args.name.strip()
        else:
            save_name = (
                f"{args.guidance_mode}_"
                + time.strftime("%Y%m%d-%H%M%S")
                + f"_{args.start_index}-{args.start_index + args.traj_length}"
            )
        out_path = args.traj_root / "egoallo_outputs" / (save_name + ".npz")
        args_out_path = args.traj_root / "egoallo_outputs" / (save_name + "_args.yaml")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        if out_path.exists() and not args.overwrite_output:
            raise FileExistsError(
                f"Output already exists: {out_path}. "
                "Use --overwrite-output to replace it or pass --name with a new value."
            )
        if args.overwrite_output:
            if out_path.exists():
                out_path.unlink()
            if args_out_path.exists():
                args_out_path.unlink()
        args_out_path.write_text(yaml.dump(dataclasses.asdict(args)))
