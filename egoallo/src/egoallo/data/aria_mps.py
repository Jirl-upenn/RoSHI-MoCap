...
def load_point_cloud_and_find_ground(
    ...
    if (
        ...
        if filtered_points_data.shape[0] == 0:
            filtered_points_data = less_filtered_points_data
        print(f"filtered_points_data: {filtered_points_data}; shape: {filtered_points_data.shape}")
        print(f"less_filtered_points_data: {less_filtered_points_data}; shape: {less_filtered_points_data.shape}")
    else:
        points_data = mps.read_global_point_cloud(str(points_path))  # type: ignore
        ...
        filtered_points_data = filter_points_from_confidence(
            points_data,
            threshold_invdep=0.0001, # for dataset 3: 0.0005
            threshold_dep=0.005, # for dataset 3: 0.02
        )
    ...
    # filtered_zs = less_filtered_points_data[:, 2]
    # print("This is the filtered points:", less_filtered_points_data)
    zs = filtered_zs
