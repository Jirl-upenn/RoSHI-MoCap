...
def do_guidance_optimization(
    ...
    # print("++++++++++++++++++++")        
    # print("Before put into guidance optimization")
    # print(f"{aria_detections.detections_left_concat.wrist_position.shape=}")
...
class JaxGuidanceParams:
    ...
    body_quat_vel_smoothness_weight: float = 15.0
    body_quat_smoothness_weight: float = 15.0#1.0
    ...
    aria_all_landmarks_weight: float = 100.0

    aria_all_hand_wrist: jdc.Static[bool] = True
    aria_all_hand_wrist_pose_weight: float = 1.0
    aria_all_hand_wrist_ori_weight: float = 1.0

    imu_readings: jdc.Static[bool] = True
    imu_local_quat_weight: float = 5.0 
    imu_pelvis_relative_rotation_weight: float = 20.0 #100.0
    imu_shoulder_relative_rotation_weight: float = 5.0 #100.0
    imu_spine_regularization_weight: float = 2.0 # 1000.0 # 1 is not good, 5 is not that goods, 10 you can keep it first
    imu_collar_regularization_weight: float = 0.0
    imu_body_prior_weight: float = 0.1
    imu_body_smoothness_weight: float = 15.0
        ...
        if mode == "no_hands":
            ...
            return {
                    "inner": JaxGuidanceParams(
                        ...
                        hand_quat_priors=True,
                        aria_wrists= True,
                        # aria_all_hand_wrist=False, #True,
                        aria_all_landmarks=True,
                        imu_readings=True,
                        max_iters=5,
                    ),
...
def _optimize(
    ...
    if imu_readings is not None and guidance_params.imu_readings:
        ...
        def spine_regularization_cost(
            ...
            spine_joint_id = [3, 6, 9]  # Assuming these are the spine indices # , 13, 14 are collar
            assert all(BODY_24_ID2NAME[joint_id] in ["spine1", "spine2", "spine3"] for joint_id in spine_joint_id), f"Spine joint ID {spine_joint_id} not found in BODY_24_ID2NAME"
        ...
        # collar regulation: penalize collar movements
        @cost_with_args(
            _SmplhBodyPosesVar(jnp.arange(timesteps)),
        )
        def collar_regularization_cost(
            vals: jaxls.VarValues,
            body_pose: _SmplhBodyPosesVar,
        ) -> jax.Array:
            """
            Penalize large movements in collar joints to prevent hunchback effect.
            This acts as a regularization term that keeps collar joints close to their initial poses.
            """
            local_quats_from_model_all = vals[body_pose]  # (21, 4)

            # Define collar joint indices (L_collar, R_collar)
            collar_joint_id = [13, 14]  # Assuming these are the spine indices # , 13, 14 are collar
            assert all(BODY_24_ID2NAME[joint_id] in ["L_collar", "R_collar"] for joint_id in collar_joint_id), f"Spine joint ID {collar_joint_id} not found in BODY_24_ID2NAME"
            collar_joint_indices = [joint_id - 1 for joint_id in collar_joint_id]

            # Get initial collar quaternions from the original trajectory
            init_collar_quats = init_quats[body_pose.id, collar_joint_indices, :]  # (3, 4)
            
            # Get current collar quaternions
            current_collar_quats = local_quats_from_model_all[collar_joint_indices, :]  # (3, 4)
            
            # Compute quaternion error between current and initial collar poses
            # init_collar_so3 = jaxlie.SO3(init_collar_quats)
            identity_so3 = jaxlie.SO3(jnp.array([1.0, 0.0, 0.0, 0.0]))
            current_collar_so3 = jaxlie.SO3(current_collar_quats)
            
            return (
                guidance_params.imu_collar_regularization_weight 
                * (current_collar_so3.inverse() @ identity_so3)
                .log()
                .flatten()
            )
        ...
        def body_quat_smoothness_first_order(
            ...
            rel = Rt.inverse() @ Rt1              
            so3 = rel.log()                      
            return guidance_params.imu_body_smoothness_weight * (so3).flatten()
    ...
    # All hand pose cost.
    if aria_all_hand_detections is not None and guidance_params.hand_quat_priors:
    ...
    if aria_all_hand_detections is not None and guidance_params.aria_all_landmarks:
        ...
        def hand_3d_alignment_cost_world(
            ...
            selected_landmarks = landmarks_3d[aria_landmark_indices]  # (15, 3)
            
            # Since there are no tips to consider, we can directly use the MANO joint positions
            # mano_hand_joints_wrt_world already has 15 joints (wrist + 14 hand joints)
            # and selected_landmarks also has 15 landmarks, so they match directly
            ...
            mano_relative = mano_hand_joints_wrt_world - wrist_pos # in world frame
            aria_relative = selected_landmarks - aria_wrist_pos # in world frame
            
            # # Method 2: Compute finger lengths (scale invariant)
            # # Define finger joint sequences (assuming standard MANO order)
            # # Note: Both mano_hand_joints_wrt_world and selected_landmarks have 15 joints (0-14)
            # finger_joints = [
            #     [0, 1, 2, 3],      # thumb
            #     [0, 4, 5, 6],      # index
            #     [0, 7, 8, 9],      # middle  
    ...
    if aria_all_hand_detections is not None and guidance_params.aria_all_hand_wrist:
        aria_all_left = aria_all_hand_detections["detections_left_concat"]
        aria_all_right = aria_all_hand_detections["detections_right_concat"]
        
        # wrist pose & position cost 
        @(
            cost_with_args(
                _SmplhBodyPosesVar(aria_all_left["indices"]),
        ...
        def wrist_pose_cost_in_hand_all_tracking(
            ...
            return confidence * jnp.concatenate(
                [
                    guidance_params.aria_all_hand_wrist_pose_weight * pose_cost,
                    guidance_params.aria_all_hand_wrist_ori_weight * ori_cost,
                ]
            )
