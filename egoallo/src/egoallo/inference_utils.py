...
class InferenceTrajectoryPaths:
    ...
    closed_loop_trajectory_csv: Path | None
    @staticmethod
    def find(traj_root: Path) -> InferenceTrajectoryPaths:
        ...
        if len(imu_readings_pkl) == 0:
            ...
            assert len(imu_readings_pkl) == 1, (
                "Found multiple IMU readings files!"
            )
        ...
        if len(hand_tracking_results_csv) == 0:
            ...
            assert len(hand_tracking_results_csv) == 1, (
                "Found multiple hand tracking results files!"
            )

        closed_loop_trajectory_csv = tuple(traj_root.glob("**/closed_loop_trajectory.csv"))
        if len(closed_loop_trajectory_csv) == 0:
            closed_loop_trajectory_csv = None
        else:
            assert len(closed_loop_trajectory_csv) == 1, (
                "Found multiple closed loop trajectory files!"
            )
